...
from src.models.sda_blocks import SDATransformerEncoderLayer, TransformerEncoder
...
# ==== Informer Variants ====
class SDAInformerMinimal(nn.Module):
...
class SDAProbSparseAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        top_u = torch.topk(scores.abs().max(dim=-1).values, u, dim=-1).indices
        ...
        for b in range(B):
            ...
            for h in range(self.n_heads):
                ...
                attn_weights = torch.softmax(attn, dim=-1) - torch.softmax(-attn, dim=-1)
...
class SDAProbSparseTransformerLayer(nn.Module):
    def __init__(self, d_model, n_heads, dim_feedforward):
        super().__init__()
        self.attn = SDAProbSparseAttention(d_model, n_heads)
...
class SDAInformerStandard(nn.Module):
    def __init__(self, patch_length, d_model=8, num_heads=2, horizon=6):
        super().__init__()
        self.embedding = nn.Linear(patch_length, d_model)
        self.pos_encoding = PositionalEncoding(d_model)
        self.encoder_layers = nn.ModuleList(
            [
                SDAProbSparseTransformerLayer(d_model, num_heads, dim_feedforward=32)
                for _ in range(2)
            ]
        )
        self.fc_out = nn.Linear(d_model, horizon)
...
class SDAFullInformer(nn.Module):
    def __init__(
        self,
        patch_length,
        horizon,
        ...
        self.encoder = nn.ModuleList(
            [
                SDAProbSparseTransformerLayer(d_model, n_heads, d_ff)
